...
| Sample | Produced by | `source_kind` · `confidence` | What it proves |
|--------|-------------|-------------------------------|----------------|
| [`two-act-spark-live-summary-sample.json`](two-act-spark-live-summary-sample.json) | `scripts/two-act-spark-proof.sh` (live `claude`, authenticated, 2026-06-10) | `collectable_v1` · `high` | **The committed live run.** A real Claude (server-resolved `claude-opus-4-8`) authored the failing patch from the underspecified spec and the fix from recorded failure evidence: act1 red **attributed to the hidden target**, act2 green with warm cache hits (`hit_rate` 0.5), live `receipt_verified_v1` receipts on both acts, M9 compare exported, prompt-redaction scan gate clean. |
| [`two-act-spark-live-receipt-sample.json`](two-act-spark-live-receipt-sample.json) | `nlfr agent-invoke` (live `claude`, authenticated) | `collectable_v1` · `high` | The act-1 receipt of the live run (`nlfr.agent_receipt.v1`): CLI name/version, sanitized command (`<prompt:sha256>` placeholder), server-resolved model id, session id, token usage, `prompt_sha256` + `response_sha256`. Raw prompt is structurally absent. |
| [`two-act-spark-stub-summary-sample.json`](two-act-spark-stub-summary-sample.json) | `scripts/two-act-spark-proof.sh` (stub CLI) | mixed: `collectable_v1` validation/cache; `simulated_v1` agent leg · `high` | Zero-token CI mechanics gate: same scenario through the deterministic stub, labeled `simulated_v1`/`stub_receipt_v1`, never presented as live. |
